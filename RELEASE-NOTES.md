...
- **SessionStart context injection** — Copilot CLI v1.0.11 added support for `additionalContext` in sessionStart hook output. The session-start hook now detects the `COPILOT_CLI` environment variable and emits the SDK-standard `{ "additionalContext": "..." }` format, giving Copilot CLI users the full agent-harness bootstrap at session start. (Original fix by @culinablaz in PR #910)
- **Tool mapping** — added `references/copilot-tools.md` with the full Claude Code to Copilot CLI tool equivalence table
- **Skill and README updates** — added Copilot CLI to the `using-agent-harness` skill's platform instructions and README installation section
...
- **Added `package.json`** so OpenCode can install agent-harness as an npm package from git.
...
- `GEMINI.md` @imports `using-agent-harness` skill and tool mapping table at session start
- Gemini CLI tool mapping reference (`skills/using-agent-harness/references/gemini-tools.md`) — translates Claude Code tool names (Read, Write, Edit, Bash, etc.) to Gemini CLI equivalents (read_file, write_file, replace, etc.)
...
- Specs (brainstorming output) now save to `docs/agent-harness/specs/YYYY-MM-DD-<topic>-design.md`
- Plans (writing-plans output) now save to `docs/agent-harness/plans/YYYY-MM-DD-<feature-name>.md`
...
Writing-plans no longer offers a choice between subagent-driven and executing-plans. On harnesses with subagent support (Claude Code, Codex), subagent-driven-development is required. Executing-plans is reserved for harnesses without subagent capability, and now tells the user that Agent Harness works better on a subagent-capable platform.
...
- `lib/brainstorm-server/` — WebSocket server with browser helper library, session management scripts, and dark/light themed frame template ("Agent Harness Brainstorming" with GitHub link)
...
- Design spec and implementation plan in `docs/agent-harness/`
...
Added explicit priority ordering to using-agent-harness:

1. User's explicit instructions (CLAUDE.md, AGENTS.md, direct requests) — highest priority
2. Agent Harness skills — override default system behavior
...
Added `<SUBAGENT-STOP>` block to using-agent-harness. Subagents dispatched for specific tasks now skip the skill instead of activating the 1% rule and invoking full skill workflows.
...
**Using-agent-harness workflow graph intercepts EnterPlanMode**
...
The `agent-harness-codex` bootstrap CLI, Windows `.cmd` wrapper, and related bootstrap content file have been removed. Codex now uses native skill discovery via `~/.agents/skills/agent-harness/` symlink, so the old `use_skill`/`find_skills` CLI tools are no longer needed.
...
The synchronous SessionStart hook blocked the TUI from entering raw mode on Windows, freezing all keyboard input. Running the hook async prevents the freeze while still injecting agent-harness context.
...
- Windows doesn't respect shebangs, so directly invoking the extensionless `agent-harness-codex` script triggered an "Open with" dialog. All invocations now prefixed with `node`.
...
Agent Harness for OpenCode now uses OpenCode's native `skill` tool instead of custom `use_skill`/`find_skills` tools. This is a cleaner integration that works with OpenCode's built-in skill discovery.

**Migration required:** Skills must be symlinked to `~/.config/opencode/skills/agent-harness/` (see updated installation docs).
...
**Strengthened using-agent-harness skill for explicit skill requests**
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-agent-harness` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-agent-harness`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `agent-harness:brainstorming`)
...
**Skill priority in using-agent-harness**
...
- **Rationalizations table** in using-agent-harness - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-agent-harness is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > agent-harness
...
- Personal skills override agent-harness skills when names match
...
- All skill references updated to use namespaced `agent-harness:code-reviewer`
- Fixes #55
...
The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([obra/agent-harness-skills](https://github.com/obra/agent-harness-skills)). This transforms agent-harness from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [obra/agent-harness-skills](https://github.com/obra/agent-harness-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/agent-harness/skills/`
...
```bash
# In Claude Code
/plugin marketplace add obra/agent-harness-marketplace
...
- Skills repository is now at https://github.com/obra/agent-harness-skills
...
**Full Changelog:** https://github.com/obra/agent-harness/compare/dd013f6...main
**Skills Repository:** https://github.com/obra/agent-harness-skills
**Issues:** https://github.com/obra/agent-harness/issues
