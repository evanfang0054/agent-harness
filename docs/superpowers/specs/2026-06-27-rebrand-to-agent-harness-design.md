# 设计：将 superpowers 重命名为 agent-harness
...
本仓库 fork 自 Jesse Vincent 的 [Superpowers](https://github.com/obra/superpowers)。经过大量自定义改动后，需将项目转变为独立项目，仅在 README 中保留对原作者的致敬，其余所有 `superpowers` 字眼一律改为 `agent-harness`。

**目标**：

- 命名空间、目录、skill 名、插件元数据、文档中所有 `superpowers` 字眼统一改为 `agent-harness`。
...
**全局统计**（spec 撰写时）：`grep -r superpowers` 命中 197 个文件、1222 处。
...
- **skill 目录**：
  ...
  - 对应测试目录 `tests/skill-behavior/using-superpowers/` 同步
- **skill 命名空间**（所有 SKILL.md 与跨 skill 调用）：
  - `superpowers:using-superpowers` → `agent-harness:using-agent-harness`
  - `superpowers:<skill>` → `agent-harness:<skill>`
- **skill 名标识符**：`using-superpowers` → `using-agent-harness`（YAML frontmatter `name` 字段、目录名、引用）
- **插件元数据**：
  - `.claude-plugin/plugin.json`：`name: superpowers` → `name: agent-harness`
  - `.claude-plugin/marketplace.json`：`superpowers-dev` → `agent-harness-dev`；其中 `name: superpowers` → `agent-harness`
  - `package.json`：`name: superpowers` → `agent-harness`；`description` 中 `Superpowers` 字样替换
  - `.codex-plugin/plugin.json`、`.pi/extensions/superpowers.ts` 同步
- **配置引用**：
  - `.claude/settings.json` / `settings.local.json.example` 中 `enabledPlugins.superpowers@superpowers-dev` → `agent-harness@agent-harness-dev`
- **hooks 与 scripts**：
  - `hooks/session-start`、`hooks/session-start-codex`、`hooks/stop-hook.sh` 中读取 `using-superpowers` 的路径同步
  - `scripts/*learnings.sh`、`scripts/auto-loop.sh`、`scripts/guard-staging.sh`、`scripts/loop-detector.sh`、`scripts/trace-analyzer.sh`、`scripts/coverage-metrics.sh`、`scripts/audit-subagent.sh`、`scripts/setup-ralph-loop.sh`、`scripts/log-learning.sh` 中 `.superpowers/` 路径引用同步
- **文档**：
  - `README.md`、`README_EN.md`：除"致敬段"外，所有 `Superpowers` / `superpowers` 替换
  ...
  - `docs/superpowers/` 下历史 specs / plans / contracts 文档（文件名与内容）：一并改名与替换
...
- `CLAUDE.md` 中"基于 Jesse Vincent 的原版 Superpowers 项目"一句（作为致敬上下文）
- `.git/` 与全部 git 历史
...
| 8 | `Superpowers` | `Agent Harness` | 大驼峰单词（标题、品牌） |
| 9 | `superpowers` | `agent-harness` | 全小写单词（兜底） |

**正则原则**：使用单词边界 `\b` 防止误伤子串。序号 1-3 必须先于序号 9 执行；序号 4-5 必须先于序号 9 执行；否则 `.superpowers/` 中的 `superpowers` 会被规则 9 错误改写但点号 `.` 与斜杠 `/` 已被词边界处理，需显式前置。
...
该段是仓库中**唯一**保留 `Superpowers` 字样的"现在时"位置（LICENSE 与 git 历史除外）。所有其他位置一律替换。`README_EN.md` 同步新增对应英文段。

`CLAUDE.md` 中"基于 Jesse Vincent 的原版 Superpowers 项目"一句保留作为致敬上下文（项目说明文档的元信息）。若用户希望在 CLAUDE.md 中也彻底去除，可在实现时调整。
...
10. **提交**：
    ```bash
    git add -A
    git commit -m "refactor: rebrand superpowers → agent-harness (preserve attribution in README)"
    ```
...
| 风险 | 缓解 |
|---|---|
| 误伤 `superpowers` 子串 | 使用 `\b` 词边界；规则 1-7 先于规则 9；预览模式让用户最后确认 |
...
- **commit message**：`refactor: rebrand superpowers → agent-harness (preserve attribution in README)`
