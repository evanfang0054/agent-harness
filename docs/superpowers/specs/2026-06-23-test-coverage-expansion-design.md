...
为 superpowers 仓库补齐测试覆盖率，确保每个公共表面（plugin 基础设施 + 所有 skill）都有测试兜底，同时保持现有运行方式（每个套件独立运行，无强制统一入口）。

## 非目标（YAGNI）

- 不引入 obra/superpowers-evals 的 `drill` harness（外部依赖、setup 重）
...
| `test-session-start-injection.sh` | `hooks/session-start` 在受控环境下能输出合法 JSON，且包含 `using-superpowers` skill 内容块 | 用管道喂一份假的 stdin JSON，捕获 stdout，jq 提取 `hookSpecificOutput.additionalContext` 包含 "using-superpowers" |
...
**职责**：
1. 在 `/tmp/superpowers-tests/<ts>/skill-behavior/<skill>/` 建独立工作目录
...
- 输入 skill-name + 日志文件
- 断言 `"name":"Skill"` 出现 且 `"skill":"superpowers:<name>"` 出现
...
- **explicit prompt**（仅核心闭环 skill）：显式 `/superpowers:<skill-name>` 调用，测 user-invocable 路径
...
| **元/Harness** | using-superpowers、writing-skills、harness-design、harness-init、harness-optimizer、computational-sensors | 触发 + 输出含对应核心概念（skills/harness/sensor/coverage） |
...
- 测试结束保留 `/tmp/superpowers-tests/<ts>/` 供事后排查（不主动清理，依赖系统 tmp 轮转）
