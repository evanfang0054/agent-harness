...
Superpowers 项目通过 session-learnings 记录经验教训，但"从会话发现问题 → 提 issue → SDD 修复 → PR"这条链路目前完全靠人肉执行（今天面板 %28 + %33 手动做了一遍，耗时且容易漏步骤）。
...
- 不做自定义 issue 模板配置（superpowers 现有模板足够）
...
所有情况下，issues 和 PR 都提到 `evanfang0054/superpowers` 仓库。
...
│ - 最保守决策原则                                         │
│                                                         │
│ 自主使用的 skills（--plugin-dir superpowers）：           │
...
[3] 分析会话    识别问题 → 分类（代码 bug / 流程问题 / skill 改进）
     ↓
[4] 提 issues   全部提到 evanfang0054/superpowers
     ↓          gh issue list 去重
...
**所有情况下，issues 和 PR 都提到 `evanfang0054/superpowers`**。
...
| 目标仓库 | 硬编码 superpowers | 自我提升工具，所有改进归本项目 |
