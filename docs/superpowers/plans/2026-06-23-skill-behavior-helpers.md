...
**Goal:** 为 28 个 skill 行为测试打地基——创建通用运行器 `run-skill.sh`、通用断言函数 `assert-skill-triggered.sh`，以及一个最简单的冒烟 skill（using-superpowers）验证整个框架可用。
...
**Spec:** `docs/superpowers/specs/2026-06-23-test-coverage-expansion-design.md`（阶段 2 部分）
...
| `tests/skill-behavior/using-superpowers/prompts/ask-about-skills.txt` | 冒烟 prompt |
| `tests/skill-behavior/using-superpowers/run-test.sh` | 冒烟 skill 测试，验证整个框架 |
...
## Task 3: 冒烟 skill — using-superpowers

**Files:**
- Create: `tests/skill-behavior/using-superpowers/prompts/ask-about-skills.txt`
- Create: `tests/skill-behavior/using-superpowers/run-test.sh`

**目的**：用一个最简单的 skill 验证整个 helpers 框架能跑通。using-superpowers 是会话启动时自动加载的入门 skill，最容易触发。

- [ ] **Step 1: 写 prompt**

`tests/skill-behavior/using-superpowers/prompts/ask-about-skills.txt`:

```
What skills are available in this project? How should I use the superpowers skills system?
```
...
assert_skill_triggered "using-superpowers"
assert_no_premature_action
...
Run:
```bash
chmod +x tests/skill-behavior/using-superpowers/run-test.sh
bash tests/skill-behavior/using-superpowers/run-test.sh
```
Expected: skill 触发断言 PASS，末尾 `STATUS: PASSED`

> **注意**：using-superpowers 的 description 是 "Use when starting any conversation"，触发率非常高。如果这个 skill 都不能触发，说明 helper 框架有问题，必须先修好。
...
3. 如果 claude 完全没调用 Skill 工具，可能是 prompt 不够明确，改成 "Use the superpowers skills system to help me understand what's available"
...
```bash
git add tests/skill-behavior/using-superpowers/
git commit -m "test(skill-behavior): 添加 using-superpowers 冒烟测试"
```
...
- 通用运行器 → Task 1 ✓
- 通用断言函数 → Task 2 ✓
- 至少一个 skill 跑通验证框架 → Task 3（using-superpowers）✓
- CLAUDE.md 更新 → Task 4 ✓
...
Plan complete and saved to `docs/superpowers/plans/2026-06-23-skill-behavior-helpers.md`. 本批是后续 3 批 skill 测试的基础设施，建议用 Subagent-Driven 模式逐 task 推进，尤其 Task 3 需要实际 headless 运行验证。
