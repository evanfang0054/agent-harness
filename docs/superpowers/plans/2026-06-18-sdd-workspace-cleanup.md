...
**Goal:** 在 finishing-a-development-branch 收尾时自动清空 `.superpowers/sdd/` 工作区，避免临时 artifacts 跨 session 累积。

**Architecture:** 新增独立 `cleanup-workspace` bash 脚本封装清理逻辑（失败不阻断），`finishing-a-development-branch` skill 在每个收尾选项末尾调用它。脚本直接拼路径 `<root>/.superpowers/sdd`，不依赖 `sdd-workspace`（避免 mkdir/mkgitignore 副作用）。

**Tech Stack:** Bash（脚本），Markdown（skill 文本改动），shell 单测（参考 `tests/learnings-scripts/test-learnings.sh` 风格）

**Spec:** `docs/superpowers/specs/2026-06-18-sdd-workspace-cleanup-design.md`
...
echo "exit=$?"
```
Expected: `exit=0`，无 stdout 输出（`.superpowers/sdd/` 不存在，静默退出）
...
touch .superpowers/sdd/.gitignore .superpowers/sdd/progress.md \
      .superpowers/sdd/task-1-brief.md .superpowers/sdd/review-aaa..bbb.diff && \
bash /Users/arwen/Desktop/Arwen/evanfang/superpowers/skills/subagent-driven-development/scripts/cleanup-workspace && \
echo "---after---" && ls -la .superpowers/sdd/
```
Expected:
- stdout 含一行 `cleaned: <绝对路径>/.superpowers/sdd`
...
清空 .superpowers/sdd/ 内容（含 .gitignore、progress、task brief/report、
...
Test dir: /tmp/superpowers-cleanup-test-XXXX
...
在 finishing-a-development-branch 收尾时自动清理 .superpowers/sdd/：
...
Plan complete and saved to `docs/superpowers/plans/2026-06-18-sdd-workspace-cleanup.md`. Two execution options:
