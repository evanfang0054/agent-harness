...
**Spec:** `docs/superpowers/specs/2026-06-26-auto-loop-skills-split-design.md`
...
source /Users/arwen/Desktop/Arwen/evanfang/superpowers/scripts/lib/state.sh
...
```bash
mkdir -p /Users/arwen/Desktop/Arwen/evanfang/superpowers/skills/generate-issues
```
...
封装 `scripts/auto-loop.sh --dry-run`：扫描 Claude Code 会话日志 → 识别问题模式 → 提交 GitHub issues 到 `evanfang0054/superpowers`。**不修复、不写代码、不提 PR。**
...
- 提交的 issues: GitHub `evanfang0054/superpowers` 仓库
...
```bash
mkdir -p /Users/arwen/Desktop/Arwen/evanfang/superpowers/skills/fix-issues-and-pr
```
...
`docs/superpowers/specs/2026-06-26-auto-loop-skills-split-design.md`
...
（执行者填写：搜索 evanfang0054/superpowers 已开放/已关闭 PR，引用相关项）
EOF
)"
```
