...
Subagent-Driven Development（SDD）在执行过程中会在 `.agent-harness/sdd/` 下累积大量短生命周期的临时 artifacts：
...
在 SDD 流程收尾时自动清理 `.agent-harness/sdd/` 工作区，让目录回到"下次 SDD 从空状态开始"的初始形态。
...
- 不清理 `.agent-harness/` 下其他子目录（如 `learnings.jsonl`、`session-analysis/`）
...
1. 通过 `git rev-parse --show-toplevel` 解析仓库根，拼接 `.agent-harness/sdd`
...
1. **目录不存在 → exit 0，无输出**
   - 临时 git repo 里 `.agent-harness/sdd/` 未建
   - 跑 cleanup → `exit 0`，stdout 空，目录仍未建

2. **目录有内容（含 `.gitignore`）→ 全部清空，保留空目录**
   - 造 `.agent-harness/sdd/{.gitignore,progress.md,task-1-brief.md,review-aaa..bbb.diff}`
...
| 失败场景 | 行为 |
|---|---|
| `.agent-harness/sdd/` 不存在 | 静默 `exit 0`（正常情况） |
...
| 路径常量 `<root>/.agent-harness/sdd` 在 `sdd-workspace` 和 `cleanup-workspace` 重复 | 两脚本都从 `git rev-parse --show-toplevel` 派生，路径漂移风险低；未来若路径变了，两处一起改。不抽公共脚本（YAGNI） |
| 跨 worktree 误清 | `git rev-parse --show-toplevel` 返回当前 worktree 根，per-worktree 天然隔离，不会影响其他 worktree 的 `.agent-harness/sdd/` |
| 用户手动跑 cleanup 清掉了想保留的产物 | git 历史保留所有 task commits，artifacts 本就是可重建的派生物；且 progress.md 是 append-only 的进度记录，重建代价低 |

## 成功标准

1. 跑完一次完整 SDD 流程（implementer → task reviewer → final review → finishing）后，`.agent-harness/sdd/` 为空目录
