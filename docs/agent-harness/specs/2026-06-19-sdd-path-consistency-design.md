...
v1 的 SDD workspace cleanup 实现后，在 fruit-shop demo（独立 git repo 嵌套在 agent-harness plugin repo 内）真实运行一次 SDD，暴露了四个相关问题：

1. **SDD 产物路径分叉**：task brief/report/diff 落在 `agent-harness/.agent-harness/sdd/`（外层），progress.md 落在 `demo/fruit-shop/.agent-harness/sdd/`（内层）。根因是各脚本独立调用 `git rev-parse --show-toplevel`，cwd 不同解析到不同 repo 根。
2. **cleanup 只清一个位置**：cleanup-workspace 也用 `git rev-parse` 解析路径，只能清掉它解析到的那个目录，另一个目录的产物（progress.md）残留。
3. **learnings.jsonl 读写分叉**：`log-learning.sh` 用相对路径 `.agent-harness/`（依赖 cwd），`search-learnings.sh` 也用相对路径。写和读在不同 cwd 时落点不同，session-start hook 读不到 log 写的内容。
...
- SDD brief/report/diff/progress/cleanup 全部读写同一个 `.agent-harness/sdd/`
- `log-learning.sh` 写的和 `search-learnings.sh` 读的是同一个 `.agent-harness/learnings.jsonl`
...
- 不清理 `.agent-harness/session-analysis/`（另一套机制，不在 SDD 范畴）
...
```markdown
# 旧
`cat "$(git rev-parse --show-toplevel)/.agent-harness/sdd/progress.md"`

# 新
`cat "${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel 2>/dev/null)}/.agent-harness/sdd/progress.md"`
```
...
SDD 执行：
  ├─ sdd-workspace        → ~/.../demo/fruit-shop/.agent-harness/sdd/
  ├─ task-brief           → 调 sdd-workspace，写入同上
  ├─ review-package       → 调 sdd-workspace，写入同上
  └─ progress.md (SKILL:290) → cat/append 同上目录

SDD 收尾：
  └─ cleanup-workspace    → 清 ~/.../demo/fruit-shop/.agent-harness/sdd/ (全部产物)

learnings 全程：
  ├─ log-learning.sh      → 写 ~/.../demo/fruit-shop/.agent-harness/learnings.jsonl
  └─ search-learnings.sh  → 读 ~/.../demo/fruit-shop/.agent-harness/learnings.jsonl
...
| `CLAUDE_PROJECT_DIR` 和 `git rev-parse` 都失败（非 git 仓库 + 无 env var） | 路径变成空字符串 + `/.agent-harness/sdd` —— 但这种场景在 Claude Code 会话内不会发生（harness 总会注入 `CLAUDE_PROJECT_DIR`） |
...
| `search-learnings.sh` 绝对路径化后，老 JSONL 文件读不到 | 不会 —— 读的是同一个 `${CLAUDE_PROJECT_DIR}/.agent-harness/learnings.jsonl`，跟 log-learning 写的位置一致 |
...
| fruit-shop 当前残留 `demo/fruit-shop/.agent-harness/sdd/progress.md`（2205 字节） | v2 实现后手动跑一次 `bash cleanup-workspace`（在 fruit-shop cwd 下），验证被清掉；若没清掉说明改动有遗漏 |

## 成功标准

1. `cleanup-workspace` 在 fruit-shop cwd 下运行，清掉 `demo/fruit-shop/.agent-harness/sdd/` 里残留的 progress.md（v1 遗留验证用例）
2. 一次完整 SDD 流程后，brief/report/diff/progress 全部落在同一个 `.agent-harness/sdd/` 目录，cleanup 能一次清干净
3. `log-learning.sh` 从任何 cwd 调用，都写到 `${CLAUDE_PROJECT_DIR}/.agent-harness/learnings.jsonl`；`search-learnings.sh` 读同一个文件
...
- **集成验证（手动）**：在 `demo/fruit-shop/` 目录下启动新 claude 会话，跑一次完整 SDD，检查：
  - 所有 SDD 产物是否都在 `demo/fruit-shop/.agent-harness/sdd/`
  - cleanup 是否一次清干净
  - log-learning 是否写到 `demo/fruit-shop/.agent-harness/learnings.jsonl`
  - stop hook 是否在第一次 stop 就检测到 promise
- **回归验证**：在 `agent-harness/` 根目录（plugin 开发场景）跑一次 SDD，检查：
  - SDD 产物在 `agent-harness/.agent-harness/sdd/`（这个仓库自己的）
  - 不影响 fruit-shop 的 `.agent-harness/sdd/`
