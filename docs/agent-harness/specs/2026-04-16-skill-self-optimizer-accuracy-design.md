...
- 相关输入：`.agent-harness/session-analysis/ac3a4a38-1ae1-4fcc-901d-929eef8e7661.json`、`.agent-harness/session-analysis/ac3a4a38-1ae1-4fcc-901d-929eef8e7661-report.md`
...
4. **跨项目 session 被模糊处理**  
   当前请求发生在 `agent-harness` 仓库，但命中的 session 文件实际属于 `hapi` 项目。若报告不显式展示这一点，用户容易把业务会话问题误认为 `agent-harness` 仓库问题。
...
- `agent-harness:systematic-debugging` 是否真的漏触发
