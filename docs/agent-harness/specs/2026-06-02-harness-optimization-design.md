...
**已有学习持久化：** `.agent-harness/learnings.jsonl`
...
- 数据源：`/tmp/agent-harness-edit-tracker/{session_id}/edits.json`
- 数据结构：
  ```json
  {
    "files": {
...
- 配置方式：项目根目录的 `.agent-harness/sensors.json`（可选）
  ```json
  {
    "sensors": [
...
**Contract 存储位置：** `docs/agent-harness/contracts/{feature-name}.contract.md`
...
- 输入数据源：
  - `.agent-harness/learnings.jsonl`（session learnings）
  - `docs/agent-harness/specs/`（历史 spec 文档）
  - 可选：`/tmp/agent-harness-edit-tracker/`（loop detector 数据）
- 分析维度：
...
预设常见技术栈的 harness 配置，降低新项目接入 agent-harness 的成本。
...
- `name: harness-init`
- `description: Use when initializing agent-harness in a new project or reconfiguring an existing project for a specific tech stack.`
...
- 扫描范围：
  ...
  - `.agent-harness/sensors.json`（如果存在）
  - `docs/agent-harness/contracts/`（如果存在）
  - `.agent-harness/learnings.jsonl`（retrospective 调用记录）
- 输出格式：
  ```
...
- `.agent-harness/learnings.jsonl` — 数据文件，不变更
- 现有 scripts — 不变更
...
**所有新增 skill 必须使用 `agent-harness:writing-skills` skill 创建。** 即遵循 TDD 流程：先运行 baseline 压力测试（RED）→ 编写 skill（GREEN）→ 闭环漏洞（REFACTOR）。不得直接手写 SKILL.md 后跳过测试。
