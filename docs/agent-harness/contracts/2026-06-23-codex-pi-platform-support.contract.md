...
- [ ] `.codex-plugin/plugin.json` 引用的 `assets/agent-harness-small.svg` 和 `assets/app-icon.png` 都实际存在
- [ ] `.pi/extensions/agent-harness.ts` 存在，`node --check .pi/extensions/agent-harness.ts` 退出码 0（注：`--check` 不解析 import，仅语法）
- [ ] `.pi/extensions/agent-harness.ts` 监听 5 个事件：`resources_discover`、`session_start`、`session_compact`、`agent_end`、`context`（逐字比对事件名）
- [ ] `.pi/extensions/agent-harness.ts` 含 `BOOTSTRAP_MARKER` 和 `EXTREMELY_IMPORTANT_MARKER` 常量
- [ ] `.pi/extensions/agent-harness.ts` 保留 `import type { ExtensionAPI } from "@earendil-works/pi-coding-agent"`（type-only）
- [ ] `package.json` 顶层含 `pi.extensions: ["./.pi/extensions/agent-harness.ts"]` 和 `pi.skills: ["./skills"]`
- [ ] `package.json` 的 `keywords` 数组含 `pi-package`
- [ ] `package.json` 不含 `main` 字段（因 fork 无 `.opencode/` 目录，避免指向不存在文件）
- [ ] `skills/using-agent-harness/references/pi-tools.md` 存在，含「调用 skill」「读写编辑文件」「运行 shell」「分发子 agent」「任务跟踪」五类映射
...
- Must support: Pi 通过 `.pi/extensions/agent-harness.ts` 在 session 启动和压缩后注入 bootstrap
...
- Inferential: 人工 diff 审查 `.pi/extensions/agent-harness.ts` 是否保留了上游的事件契约（5 个事件 + 去重 + 插入位置）
