...
**Goal:** 基于 spec `docs/agent-harness/specs/2026-06-02-harness-optimization-design.md` 实施 P0-P3 全部 7 个优化方向，覆盖 loop detection、computational sensors、sprint contract、trace analysis、FF/FB 分类、harness templates、coverage metrics。
...
**Spec 参考：** `docs/agent-harness/specs/2026-06-02-harness-optimization-design.md`
...
Run: `rm -rf /tmp/agent-harness-edit-tracker/test-smoke`
...
Tracks per-file edit counts via /tmp/agent-harness-edit-tracker/{session}/edits.json.
...
**约束：** 必须使用 `agent-harness:writing-skills` skill 创建（TDD 流程：RED baseline → GREEN skill → REFACTOR）。
...
调用 `agent-harness:writing-skills` skill，基于 baseline 中观察到的具体失败模式编写 `skills/loop-detection/SKILL.md`。
...
调用 `agent-harness:writing-skills` skill 编写 `skills/computational-sensors/SKILL.md`。
...
- `.agent-harness/sensors.json` 配置格式
...
Configuration via .agent-harness/sensors.json.
...
- 存储位置：`docs/agent-harness/contracts/{feature-name}.contract.md`
...
Contract stored at docs/agent-harness/contracts/{feature}.contract.md.
...
Expected: 输出 `=== Trace Analysis Report ===` 和分析结果（基于当前 `.agent-harness/learnings.jsonl` 的 7 条记录）
...
Parses .agent-harness/learnings.jsonl and classifies failure patterns:
...
场景：用户在新项目中安装了 agent-harness，不知道该启用哪些 skill 和配置哪些 sensor。
...
Lists available templates and copies sensors.json to .agent-harness/."
```
...
**3. Type consistency:** 检查脚本函数名、文件路径、env var 名称跨 task 一致——`LOOP_WARN_THRESHOLD`、`CLAUDE_SESSION_ID`、`.agent-harness/sensors.json` 等均一致。
