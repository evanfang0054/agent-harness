# Agent Harness → Agent Harness 重命名实施计划
...
**Goal:** 将仓库内所有 `agent-harness` 字眼（命名空间、目录、skill 名、插件元数据、文档）重命名为 `agent-harness`，仅在 README 保留对原作者 Jesse Vincent 的致敬段。
...
```bash
#!/usr/bin/env bash
# Rebrand: agent-harness → agent-harness
# Spec: docs/agent-harness/specs/2026-06-27-rebrand-to-agent-harness-design.md
# (注：脚本编写时 docs/agent-harness 尚未重命名，运行时第3阶段会同步改名)
set -euo pipefail
...
# demo 子项目（先改，避免主项目 docs/agent-harness 先改后路径歧义）
[[ -d "demo/fruit-shop/.agent-harness" ]] && git mv "demo/fruit-shop/.agent-harness" "demo/fruit-shop/.agent-harness"
[[ -d "demo/fruit-shop/docs/agent-harness" ]] && git mv "demo/fruit-shop/docs/agent-harness" "demo/fruit-shop/docs/agent-harness"
[[ -d "demo/docs/agent-harness" ]] && git mv "demo/docs/agent-harness" "demo/docs/agent-harness"

# 主项目顶层目录
[[ -d "docs/agent-harness" ]] && git mv "docs/agent-harness" "docs/agent-harness"
[[ -d ".agent-harness" ]] && git mv ".agent-harness" ".agent-harness"

# skill 目录与测试目录
[[ -d "skills/using-agent-harness" ]] && git mv "skills/using-agent-harness" "skills/using-agent-harness"
[[ -d "tests/skill-behavior/using-agent-harness" ]] && git mv "tests/skill-behavior/using-agent-harness" "tests/skill-behavior/using-agent-harness"

# pi 扩展文件
[[ -f ".pi/extensions/agent-harness.ts" ]] && git mv ".pi/extensions/agent-harness.ts" ".pi/extensions/agent-harness.ts"
...
# 统计含 agent-harness 的文件（不区分大小写）
AFFECTED=$(echo "$FILES_TO_PROCESS" | xargs grep -li 'agent-harness' 2>/dev/null | wc -l | tr -d ' ')
echo "  将处理 $AFFECTED 个文件"
echo "$FILES_TO_PROCESS" | xargs grep -li 'agent-harness' 2>/dev/null | head -30
echo "..."
echo ""
...
# 规则 1（复合 token）
apply_sed 'agent-harness:using-agent-harness' 'agent-harness:using-agent-harness' '规则1: 复合 token'

# 规则 2（命名空间）
apply_sed 'agent-harness:' 'agent-harness:' '规则2: 命名空间 agent-harness: → agent-harness:'

# 规则 3（skill 名标识符）
apply_sed 'using-agent-harness' 'using-agent-harness' '规则3: skill 名 using-agent-harness'

# 规则 4（隐藏目录路径）
apply_sed '\.agent-harness/' '.agent-harness/' '规则4: 路径 .agent-harness/'
...
This project is forked from Jesse Vincent ([@obra](https://github.com/obra))\'s [Agent Harness](https://github.com/obra/agent-harness). We retain its workflow philosophy and skill architecture. Credit to the original author.'
...
# CLAUDE.md：保留"基于 Jesse Vincent 的原版 Agent Harness 项目"一句作为致敬上下文
# 其余 agent-harness 字眼做替换
if [[ -f "CLAUDE.md" ]]; then
    # 用占位符保护致敬句
    sed "${SED_INPLACE[@]}" -E 's/基于 Jesse Vincent 的原版 Agent Harness 项目/__TRIBUTE_SENTINEL__/g' CLAUDE.md
...
# --- 阶段 5：残留扫描 ---
echo "▶ 阶段 5/5：残留 agent-harness 字眼扫描"

# 白名单：LICENSE、README.md、README_EN.md、CLAUDE.md（致敬段）
RESIDUAL=$(grep -rni 'agent-harness' \
    --exclude-dir=.git \
    --exclude=LICENSE \
    --exclude=README.md \
...
if [[ -n "$RESIDUAL" ]]; then
    echo "⚠️  发现残留 agent-harness 字眼（非白名单）：" >&2
    echo "$RESIDUAL" >&2
    echo "" >&2
...
# 致敬段位置确认
echo ""
echo "📋 致敬段确认（应保留 Agent Harness 字样）："
echo "--- README.md ---"
...
```bash
for p in \
    "demo/fruit-shop/.agent-harness" \
    "demo/fruit-shop/docs/agent-harness" \
    "demo/docs/agent-harness" \
    "docs/agent-harness" \
    ".agent-harness" \
    "skills/using-agent-harness" \
    "tests/skill-behavior/using-agent-harness" \
    ".pi/extensions/agent-harness.ts"; do
...
Run:
```bash
echo "规则1 复合 token:  $(grep -rl 'agent-harness:using-agent-harness' --exclude-dir=.git . | wc -l | tr -d ' ')"
echo "规则2 命名空间:    $(grep -rl 'agent-harness:' --exclude-dir=.git . | wc -l | tr -d ' ')"
echo "规则3 skill名:     $(grep -rl 'using-agent-harness' --exclude-dir=.git . | wc -l | tr -d ' ')"
...
grep -rlE '\b(agent-harness|Agent Harness)\b' --exclude-dir=.git . \
...
**失败处理**：
- 若 `test-session-start-injection.sh` 失败：检查 `hooks/session-start` 中 `${PLUGIN_ROOT}/skills/using-agent-harness/SKILL.md` 是否已改为 `${PLUGIN_ROOT}/skills/using-agent-harness/SKILL.md`，以及 `.agent-harness/learnings.jsonl` 是否已改为 `.agent-harness/learnings.jsonl`
...
```
Expected: 标题下方紧跟「## 致谢 / Acknowledgements」段，内容含 `Jesse Vincent`、`@obra`、`Agent Harness`（链接到 obra/agent-harness）。无其他 `Agent Harness` 字样（除致谢段）。

- [ ] **Step 11.2: review CLAUDE.md 致敬句**

Run:
```bash
grep -n 'Jesse Vincent\|Agent Harness\|agent-harness' CLAUDE.md
```
Expected: 仅命中「基于 Jesse Vincent 的原版 Agent Harness 项目」一句；其余原 `Agent Harness` 字样已替换为 `Agent Harness` / `agent-harness`。

- [ ] **Step 11.3: review hooks/session-start 关键引用**

Run:
```bash
grep -n 'using-agent-harness\|using-agent-harness\|\.agent-harness\|\.agent-harness' hooks/session-start
```
Expected: 全部为 `using-agent-harness` 与 `.agent-harness/`，无 `using-agent-harness` 或 `.agent-harness/` 残留。
...
refactor: rebrand agent-harness → agent-harness (preserve attribution in README)

- 全量重命名 agent-harness → agent-harness（命名空间、目录、skill 名、插件元数据、文档）
- skills/using-agent-harness/ → skills/using-agent-harness/
- .agent-harness/ → .agent-harness/，docs/agent-harness/ → docs/agent-harness/（含 demo 子项目）
- plugin.json / marketplace.json / package.json / settings.json 同步更新
- README.md / README_EN.md 保留对原作者 Jesse Vincent (@obra) 的致谢段
- CLAUDE.md 保留"基于 Jesse Vincent 的原版 Agent Harness 项目"致敬句
...
询问用户："是否推送并重命名 GitHub 远程仓库（evanfang0054/agent-harness → evanfang0054/agent-harness）？"
- 用户明确同意后继续
...
```
Expected: `✓ Renamed repository evanfang0054/agent-harness to evanfang0054/agent-harness`
...
```bash
gh pr create --title "refactor: rebrand agent-harness → agent-harness" --body "$(cat <<'EOF'
## Summary
- 全量重命名 agent-harness → agent-harness（命名空间、目录、skill 名、插件元数据、文档）
...
- [ ] 所有 JSON 配置 `jq .` 合法
- [ ] `grep -rni agent-harness`（除白名单 LICENSE / README.md / README_EN.md / CLAUDE.md）输出为空
- [ ] README.md / README_EN.md 含致谢段，含 `Jesse Vincent`、`@obra`、`Agent Harness`（链接）
