...
**Goal:** 让本 fork 在 Codex（App + CLI）和 Pi 两个 harness 上具备 skill 自动加载和 bootstrap 注入能力，与上游 obra/agent-harness v6.0.3 行为一致。

**Architecture:** 严格遵循上游三不变量——skills 平台无关（复用现有 `skills/`）、tool mapping 平台特定（新增 `pi-tools.md`）、bootstrap 平台特定（Codex 复用已有 shell hook，Pi 新增 TS 扩展）。Codex 补 `.codex-plugin/plugin.json` manifest + assets；Pi 新增 `.pi/extensions/agent-harness.ts` + `package.json` 的 `pi` 字段。

**Tech Stack:** Bash + jq（manifest 测试）、Node.js test runner + assert（Pi 扩展测试）、TypeScript type-only import（Pi 扩展）。

**Contract:** `docs/agent-harness/contracts/2026-06-23-codex-pi-platform-support.contract.md`
**Spec:** `docs/agent-harness/specs/2026-06-23-codex-pi-platform-support-design.md`
...
- Create: `assets/agent-harness-small.svg`
...
```bash
mkdir -p assets
cp /tmp/obra-agent-harness/assets/app-icon.png assets/app-icon.png
cp /tmp/obra-agent-harness/assets/agent-harness-small.svg assets/agent-harness-small.svg
```

如果 `/tmp/obra-agent-harness` 不存在（会话重启），先克隆：
```bash
git clone --depth 1 https://github.com/obra/agent-harness.git /tmp/obra-agent-harness
```
...
Run:
```bash
ls -l assets/app-icon.png assets/agent-harness-small.svg
```
Expected: 两个文件都存在，`app-icon.png` 大小约 48KB，`agent-harness-small.svg` 约 1.7KB。

- [ ] **Step 3: Commit**

```bash
git add assets/app-icon.png assets/agent-harness-small.svg
...
```json
{
  ...
  "homepage": "https://github.com/evanfang0054/agent-harness",
  "repository": "https://github.com/evanfang0054/agent-harness",
  "license": "MIT",
  "keywords": [
    "brainstorming",
  ...
  "interface": {
    ...
    "longDescription": "通过 Agent Harness 引导 agent 完成 brainstorming、实现规划、测试驱动开发、系统化调试、并行执行、代码审查和分支收尾工作流。",
    ...
    "websiteURL": "https://github.com/evanfang0054/agent-harness",
...
```
Expected: 分别输出 `5.0.21-beta.5` / `./skills/` / `./hooks/hooks-codex.json` / `./assets/agent-harness-small.svg`。
...
## Task 3: 新增 Pi 扩展 `.pi/extensions/agent-harness.ts`

**Files:**
- Create: `.pi/extensions/agent-harness.ts`
...
写入 `.pi/extensions/agent-harness.ts`：
...
const bootstrapSkillPath = resolve(skillsDir, "using-agent-harness", "SKILL.md");
...
The using-agent-harness skill content is included below and is already loaded for this Pi session. Follow it now. Do not try to load using-agent-harness again.
...
Pi has native skills but does not expose Claude Code's \`Skill\` tool. When a Agent Harness instruction says to invoke a skill, use Pi's native skill system instead: load the relevant \`SKILL.md\` with \`read\` when the skill applies, or let a human invoke \`/skill:name\` explicitly.

Pi's built-in coding tools are lowercase: \`read\`, \`write\`, \`edit\`, \`bash\`, plus optional \`grep\`, \`find\`, and \`ls\`. Use those for the corresponding actions: read a file, create or edit files, run shell commands, search file contents, find files by name, and list directories.

Pi does not ship a standard subagent tool. If a subagent tool such as \`subagent\` from \`pi-subagents\` is available, use it for Agent Harness subagent workflows. If no subagent tool is available, do the work in this session or explain the missing capability instead of inventing \`Task\` calls.
...
Run:
```bash
node --check .pi/extensions/agent-harness.ts
```
...
Run:
```bash
grep -c 'pi.on("resources_discover"' .pi/extensions/agent-harness.ts
grep -c 'pi.on("session_start"' .pi/extensions/agent-harness.ts
grep -c 'pi.on("session_compact"' .pi/extensions/agent-harness.ts
grep -c 'pi.on("agent_end"' .pi/extensions/agent-harness.ts
grep -c 'pi.on("context"' .pi/extensions/agent-harness.ts
grep -c 'BOOTSTRAP_MARKER' .pi/extensions/agent-harness.ts
grep -c 'EXTREMELY_IMPORTANT_MARKER' .pi/extensions/agent-harness.ts
grep -c 'import type { ExtensionAPI }' .pi/extensions/agent-harness.ts
```
...
```bash
git add .pi/extensions/agent-harness.ts
git commit -m "feat(pi): 新增 .pi/extensions/agent-harness.ts 事件驱动 bootstrap 注入"
```
...
```
Expected: 输出 `pi-package` / `./.pi/extensions/agent-harness.ts` / `./skills` / `true`（`.main | not` 为 true 表示该字段不存在）。
...
## Task 5: 新增 `skills/using-agent-harness/references/pi-tools.md`

**Files:**
- Create: `skills/using-agent-harness/references/pi-tools.md`

- [ ] **Step 1: 写入 pi-tools.md**

写入 `skills/using-agent-harness/references/pi-tools.md`：
...
Run:
```bash
grep -c "调用 skill" skills/using-agent-harness/references/pi-tools.md
grep -c "读取文件" skills/using-agent-harness/references/pi-tools.md
grep -c "运行 shell" skills/using-agent-harness/references/pi-tools.md
grep -c "分发子 agent" skills/using-agent-harness/references/pi-tools.md
grep -c "任务跟踪" skills/using-agent-harness/references/pi-tools.md
```
Expected: 每条至少 `1`。
...
```bash
git add skills/using-agent-harness/references/pi-tools.md
...
const extensionPath = resolve(repoRoot, '.pi/extensions/agent-harness.ts');
const piToolsPath = resolve(repoRoot, 'skills/using-agent-harness/references/pi-tools.md');
...
test('package.json declares a pi package with skills and extension resources', async () => {
  ...
  assert.equal(pkg.name, 'agent-harness');
  assert.ok(pkg.keywords.includes('pi-package'));
  assert.deepEqual(pkg.pi.skills, ['./skills']);
  assert.deepEqual(pkg.pi.extensions, ['./.pi/extensions/agent-harness.ts']);
});
...
test('startup context injects the bootstrap as one user message until agent_end', async () => {
  ...
  assert.match(textOf(result.messages[0]), /You have agent-harness/);
  ...
  assert.match(textOf(repeatedProviderRequest.messages[0]), /You have agent-harness/);
...
test('session_compact injects bootstrap after compaction summaries, not before compaction', async () => {
  ...
  assert.match(textOf(result.messages[1]), /You have agent-harness/);
...
ls -l assets/agent-harness-small.svg assets/app-icon.png
...
grep -c "调用 skill" skills/using-agent-harness/references/pi-tools.md
grep -c "读写\|读取文件" skills/using-agent-harness/references/pi-tools.md
grep -c "运行 shell" skills/using-agent-harness/references/pi-tools.md
grep -c "分发子 agent" skills/using-agent-harness/references/pi-tools.md
grep -c "任务跟踪" skills/using-agent-harness/references/pi-tools.md
