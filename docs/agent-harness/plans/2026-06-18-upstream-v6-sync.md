...
**Goal:** 把上游 obra/agent-harness v5.1.0 / v6.0.0 中 SDD 审查机制重写 + bug/安全修复同步到 fork，完整保留 fork 的 ralph-loop 驱动与 learnings 体系。
...
**Spec:** `docs/agent-harness/specs/2026-06-18-upstream-v6-sync-design.md`
...
skills/subagent-driven-development/scripts/task-brief \
  docs/agent-harness/plans/2026-06-18-upstream-v6-sync.md 1 /tmp/test-brief.md
...
来源：obra/agent-harness v6.0.2 (b62616f)"
```
...
**Merge 策略**：fork 的 session-start 76 行，包含三块 fork 专属逻辑必须保留：(1) legacy skills 目录警告；(2) `.agent-harness/learnings.jsonl` 注入；(3) Cursor/Claude Code/Copilot 三平台分支输出。上游 v5.1 把 Codex 分支拆到了独立的 `session-start-codex`（Task 2 已新增），所以主 session-start **不需要** 再处理 Codex。
