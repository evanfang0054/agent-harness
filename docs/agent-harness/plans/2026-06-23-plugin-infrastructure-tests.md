...
**Goal:** 为 agent-harness 仓库新建 `tests/plugin-infrastructure/` 套件，9 个纯脚本测试 + 1 个 runner，秒级覆盖 hooks 配置、session-start 注入、plugin/marketplace manifest、commands/agents frontmatter、scripts 工具脚本。
...
**Spec:** `docs/agent-harness/specs/2026-06-23-test-coverage-expansion-design.md`（阶段 1 部分）
...
| `tests/plugin-infrastructure/test-session-start-injection.sh` | 验证 `hooks/session-start` 在 mock 环境下输出合法 JSON 且包含 using-agent-harness |
...
**参考输入**（已确认）：
- `.claude-plugin/plugin.json` 字段：`name=agent-harness`、`version=5.0.21-beta.5`、`keywords` 是数组、`homepage`/`repository` 非空
- `package.json` 同名字段：`name=agent-harness`、`version=5.0.21-beta.5`
...
**参考输入**（已确认）：
- `hooks/session-start` 从 stdin 读 JSON（含 `session_id`），输出含 `hookSpecificOutput.additionalContext`，其中应包含 `using-agent-harness` skill 内容
...
Plan complete and saved to `docs/agent-harness/plans/2026-06-23-plugin-infrastructure-tests.md`. Two execution options:
