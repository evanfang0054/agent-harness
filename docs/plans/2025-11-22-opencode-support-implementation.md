...
> **For agentic workers:** REQUIRED SUB-SKILL: Use agent-harness:executing-plans to implement this plan task-by-task.

**Goal:** Add full agent-harness support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/agent-harness-codex` (lines 40-74)
...
- Reference: `.codex/agent-harness-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'agent-harness' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/agent-harness-codex` (lines 212-280)
 ...
 * (personal skills override agent-harness skills).
 *
 * @param {string} skillName - Name like "agent-harness:brainstorming" or "my-skill"
 * @param {string} superpowersDir - Path to agent-harness skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superpowersDir, personalDir) {
    // Strip agent-harness: prefix if present
    const forceSuperpowers = skillName.startsWith('agent-harness:');
    const actualSkillName = forceSuperpowers ? skillName.replace(/^agent-harness:/, '') : skillName;

    // Try personal skills first (unless explicitly agent-harness:)
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/agent-harness-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/agent-harness-codex` (add import at top)
...
Run: `node -c .codex/agent-harness-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/agent-harness-codex
...
**Files:**
- Modify: `.codex/agent-harness-codex` (lines 40-74)
...
Run: `.codex/agent-harness-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/agent-harness-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/agent-harness-codex` (lines 16-38, approximately)
...
Run: `.codex/agent-harness-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/agent-harness.js`
...
/**
 * Agent Harness plugin for OpenCode.ai
 *
...
const superpowersSkillsDir = path.join(homeDir, '.config/opencode/agent-harness/skills');
...
Run: `ls -l .opencode/plugin/agent-harness.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/agent-harness.js
...
**Files:**
- Modify: `.opencode/plugin/agent-harness.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "agent-harness:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > agent-harness)
...
Run: `node -c .opencode/plugin/agent-harness.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the agent-harness and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          ...
          if (allSkills.length === 0) {
            return 'No skills found. Install agent-harness skills to ~/.config/opencode/agent-harness/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'agent-harness:';
...
```javascript
    'session.started': async () => {
      // Read using-agent-harness skill content
...
**Skills naming:**
- Agent Harness skills: \`agent-harness:skill-name\` (from ~/.config/opencode/agent-harness/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/agent-harness')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/agent-harness && git pull` to update agent-harness.' :
        '';
...
**Below is the full content of your 'agent-harness:using-agent-harness' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
```markdown
# Installing Agent Harness for OpenCode
...
### 1. Install Agent Harness Skills

```bash
# Clone agent-harness skills to OpenCode config directory
mkdir -p ~/.config/opencode/agent-harness
git clone https://github.com/obra/agent-harness.git ~/.config/opencode/agent-harness
```

### 2. Install the Plugin

The plugin is included in the agent-harness repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/agent-harness/.opencode/plugin/agent-harness.js`
...
ln -s ~/.config/opencode/agent-harness/.opencode/plugin/agent-harness.js .opencode/plugin/agent-harness.js
```
...
```
use use_skill tool with skill_name: "agent-harness:brainstorming"
```
...
Personal skills override agent-harness skills with the same name.

## Updating

```bash
cd ~/.config/opencode/agent-harness
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/agent-harness/.opencode/plugin/agent-harness.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/agent-harness/skills`
...
- Report issues: https://github.com/obra/agent-harness/issues
- Documentation: https://github.com/obra/agent-harness
```
...
Agent Harness works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
**Files:**
- Test: `.codex/agent-harness-codex`
...
Run: `.codex/agent-harness-codex use-skill agent-harness:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/agent-harness-codex bootstrap | head -30`
...
ls -l .opencode/plugin/agent-harness.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/agent-harness.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/agent-harness-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/agent-harness-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/agent-harness.js` created with tools and hooks
